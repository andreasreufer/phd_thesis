...
for ( sparm, axv ) in params:
  ...
  for i in range(0, len(zl) ):
    ...
    filt = []
    if impa > impacrit:
      filt = ( np.array( xll[i] ) > 2.0 )
    else:
      filt = ( np.array( xll[i] ) > 0.0 )

    xarr = np.array( xll[i] ).compress( filt )
    yarr = np.array( resll[i] ).compress( filt )

    if filt.any():
      coeffs = np.polyfit( xarr, yarr, 2 )
      xth = np.linspace( 1.0, 4.0, 100 )
      ax.plot( xth, np.polyval( coeffs, xth ), "-", linewidth=0.3, color=col )
      
      (a,b,c) = coeffs
      vcritA = ( -b - sqrt( b*b - 4.*a*(c-0.5) ) ) / (2.*a)
      vcritB = ( -b + sqrt( b*b - 4.*a*(c-0.5) ) ) / (2.*a)
