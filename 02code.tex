...
In case gravity is important, thing's becomes more complicated. The motion of the impactor relative to the target will no longer follow a straight, un-accelerated trajectory. Instead the impactor follows an orbit around the target, ultimately hitting it. The relative motion between the impactor with mass $M_{imp}$ and the target $M_{tar}$

% mimp + mtar collisions equals a collisions in the target rest frame with orbiting body with mu
% r_imp = R_imp + R_tar
% impact angle, velocity, masses -> theta_imp, e, rp -> r0, theta0, beta0, v0 -> shift & turn frame, such as p0 = 0 and impact axis aligned (theta_im = pi/2)

\cite{thomson1986introduction}
