...
\caption{The same tables as in figure \ref{ch02_fig04a} for $H_2 O$ }
\label{ch02_fig04b}
\end{center}
\end{figure}
...
\caption{The same tables as in figure \ref{ch02_fig04a} for \emph{dunite}. In the grey zones, ANEOS returns valid pressures and sound speeds, but does not return any particular phase information.}
\label{ch02_fig04c}
\end{center}
\end{figure}
...
\caption{The same tables as in figure \ref{ch02_fig04a} for the modified M-ANEOS for $SiO_2$}
\label{ch02_fig04d}
\end{center}
\end{figure}
...
\subsection{Barnes \& Hut Tree algorithm}
...
Figure \ref{ch02_fig09} shows the relative acceleration errors and the speed-up for a self-gravitating sphere when using the Barnes \& Hut tree instead of direct summation. The errors are in the range of $10^{-3}$ for most particles, while the speed-up is considerably even for moderate number of particles.
...
\subsection{Neighbor search with the tree}
...
\subsection{Optimizing the code}
...
\subsection{Particles distributions for SPH spheres}
...
\end{align}
The initial time can be calculated via the mean anomaly $\Psi$, which links true anomaly with time. Together with the semi-major axis $a$, time is given by
\begin{align}
