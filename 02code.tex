...
Although the parallelization itself is simple, running it fast is not so easy. The problem is, that all workers have to access the nodes of the tree and share its data. The performance of nowadays computers is limited by the \emph{von Neumann bottleneck} \citep{bryant2010computer}: While even most complex floating point operations like divisions can be completed in only a few clock cycles, accessing the main memory takes between 50-300 cycles \citep{2008WhitepaperFry}.
