...
In case of small spheres which are not self-gravitating, the structure of the sphere is trivial. Pressure vanishes 


the particle mass is set so that the SPH density sum \ref{ch02_sph02_eq001} leads to the reference density $\rho_0$ for which the pressure vanishes. 


Setting up one-dimensional profiles is non-trivial in case 

\citep{Benz:1991p700}



% vanilla sphere + 1D structure -> flavoured body -> relax
% use of spheres for simplicity reason


\subsection{flavoring SPH spheres}
In case of small spheres which are not self-gravitating, setting the physical quantities is trivial: If standard SPH is going to be used, dividing the required density $\rho_i$ with the particle density $\delta_i$ obtained with SPH sum \ref{ch02_sph01_eq030} leads to the particle mass $m_i$. The inverse of the particle density is the particle volume.



\subsection{putting an atmosphere on top}

\subsection{relaxing SPH bodies}
% show theoretical vs. unevolved vs. evolved densities
