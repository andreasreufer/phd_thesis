...
Figure \ref{ch02_grav02_fig12} now shows a summary of the building blocks of the parallel implementation: After determining the cost zones, the particles are exchanged to their assigned domain. Note that this is not only necessary at the very first derivation step, but at each step, as particles do move and change the computational domains. Additionally, computational domains are adapted after each derivation step. In the next step, information about the ghost particles is exchanged, which is required for building the local tree and for the first SPH sum. Apart from position and mass, which is required for building the tree, also the smoothing length is required for example, assuming that the first SPH determines the density. After building the local tree by inserting local and ghost particles, the top tree is globally summed up, which requires globally summing up the multipole moments of the top tree. At this stage, everything is ready for calculating the derivatives. 
...
\caption{Relative computation cost per particle in arbitrary units. Particles inside large clumps require }
\label{ch02_grav02_fig14}
\end{center}
\end{figure}
