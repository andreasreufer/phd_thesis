...
\end{align}
where the mutual escape velocity is given by $\vesc = \sqrt{ 2 G \frac{\Mtar + \Mimp}{\Rtar + \Rimp} }$ and constant density bodies are assumed \footnote{Obviously the assumption of constant density is wrong for self-gravitating bodies, as pressure and density increase towards the center of this bodies. Nevertheless the core density is still within the same magnitude as the surface density for terrestrial bodies. Therefore this effect can be neglected in this order of magnitude calculation.}.
Gravitational restitution happens on the gravitational timescale given by
\begin{align}
...
\Mlr(Q^*_D) = 0.5 \Mtar
\end{equation}
Originally the Q-value was used to determine the specific energy required to destroy asteroids by considerably smaller impactors. In similar-sized collision the impactor is in the same mass regime as the target and the collision can also be regarded as the larger target destroying the smaller impactor. A comparable value which is symmetric in target and impactor is therefore more suited for similar sized collision. 

More recent work on similar-sized collisions \citep{Stewart:2009p3265, 2009ApJ...700L.118M, 2010ApJ...712L..73M, Leinhardt:2011p4060} uses the reduced mass and defines the specific reduced impact energy
\begin{equation}
\label{ch03_eq005}
...
Ejecta denotes the mass of the particles being ejected and not gravitationally bound to any remnant clump. The physical interpretation of those particles is not straight-forward. Depending on the thermodynamics of the ejecta, it can consist of strengthless solids in the form of debris, a two-phase fluid of liquid and vapor or simply vapor. \cite{Anic:2006p99} modeled the fate of the ejecta in high velocity collisions which probably led to todays Mercury. His collisions are in the mass regime of $\Mtar = 0.1 \ME$. He found that at first the ejecta consists of a rapidly expanding vapor plume of hot mantle material, which later on condenses into small droplets. Estimating the ejecta mass in similar-sized collisions becomes interesting in the light of recent observations of probable giant collisions in extrasolar systems \citep{Lisse:2009p3131}. It allows to constrain the collision parameters with for the observed ejecta properties.

It is important to note, that radiation transport might become important in the high target mass regime ($\Mtar > 0.1 \ME$), where for high enough impact velocities shocks become strong and ejected mantle material can vaporize. The ejection itself is mainly a hydrodynamical process and radiative transport probably can be neglected in the optically thick ejection plume on impact timescales $\tau_{coll}$. But as soon as the ejecta plume becomes optically sufficiently thin, radiation transfer might play a role in the expansion of the plume. \cite{Thompson:1988p3451} gives a cooling timescale for a thin silicate vapor disk 
\begin{align}
...
Similarly as when comparing the range of largest remnant and second remnant masses, the change in composition can be much more radical for the second remnant than for the largest remnant. Analogously to the accretion efficiency $\xi$, a stripping efficiency for the second remnant can be defined
\begin{align}
\delta = \frac{\Mimp - \Msr}{\Mimp}
\end{align}
A stripping efficiency $\delta = 0$ describes the case, where the impactor looses no mass during the collision and forms a second remnant of the same mass. A $\delta = 1$ means, that the impactor has been stripped of its mass completely and $\Msr = 0$. As the second remnant does not accrete mass during a collision, compositional changes from the impactor to the second remnant can only by achieved by different mass losses for different material or in other words by different stripping efficiencies for different materials. Figures \ref{ch03_fig24a} and \ref{ch03_fig24b} show the difference in the stripping efficiency for the second remnant for different materials. In order to yield mostly positive values, the stripping efficiency of more dense material is subtracted from the one of the less dense material. Two possible possible outcomes can be observed: For impact velocities high enough ($ > 1.30 \vesc$), the impactor forms a second remnant with parts of its mantle removed. The difference of the stripping efficiency of the materials reaches a maximum for a certain impact angle and decreases again. This angle depends on the geometry of the collision and how the different material layers pass each other. Therefore the mass ratio but also the composition determines this angle. For simulation set \css this angle is roughly $30 \deg$ for $\gamma = 0.7$, $\approx 38 \deg$ for $\gamma = 0.35$, $\approx 45 \deg$ for $\gamma = 0.2$ and $\approx 60 \deg$ for $\gamma = 0.1$. If the impact angle becomes again smaller than this angle, the impactor core also looses mass efficiently and the difference of the stripping efficiency becomes smaller again. The same effect can be seen for the icy body simulations. For $\gamma = 0.2$, the difference in silicate versus iron stripping efficiency reaches the largest difference around $30 \deg$, whereas for the difference between water ice and silicate is largest around $45 \deg$. In all this cases, the impactors tends to loose its lighter material which are nearer to the surface. This effect appears to be scale-independent, although in the larger target regime, the difference in stripping efficiencies tends to have slightly higher value at the maximum. This can be explained by stronger shocks and by low-density surface material to be more easily ejected from the impactors surface. 
...
\caption{Difference of silicate mantle and iron core stripping efficiency as a function of the impact angle for different impact velocities in simulation set \css. Only simulations with a second remnant mass $\Msr > 0.05 \Mimp$ are shown. The black dashed line gives the relative impactor volume hit by the target, assuming a straight trajectory of the impactor after the time of impact.}
\label{ch03_fig24a}
\end{center}
\end{figure}
...
\caption{Differences of water vs. silicate mantle stripping efficiency and silicate mantle vs. iron core stripping efficiency as a function of the impact angle for different impact velocities in simulation set \iss. Only simulations with a second remnant mass $\Msr > 0.05 \Mimp$ are shown. The black dashed line gives the relative impactor volume hit by the target, assuming a straight trajectory of the impactor after the time of impact.}
\label{ch03_fig24b}
\end{center}
\end{figure}
...
% dynamical effects 
%% 
\subsection{Dynamical effects: deflection and slow-down of the impactor}
...
\begin{figure}
\begin{center}
\includegraphics[scale=1.0]{22_SR_vartheta_r3.pdf}
\caption{Difference between the actual deflection angle of the second remnant $2 \vartheta_{sr}$ and the expected deflection angle $2 \vartheta$ for a purely gravitational interaction between the impactor and the target assuming point masses for simulation set \rss. Only simulations for which $\Msr > 0.1 \Mimp$.}
\label{ch03_fig22a}
\end{center}
\end{figure}
...
\caption{Difference between the actual deflection angle of the second remnant $2 \vartheta_{sr}$ and the expected deflection angle $2 \vartheta$ for a purely gravitational interaction between the impactor and the target assuming point masses for simulation set \css. Only simulations for which $\Msr > 0.1 \Mimp$.}
\label{ch03_fig22b}
\end{center}
\end{figure}
...
\caption{Difference between the actual deflection angle of the second remnant $2 \vartheta_{sr}$ and the expected deflection angle $2 \vartheta$ for a purely gravitational interaction between the impactor and the target assuming point masses for simulation set \iss. Only simulations for which $\Msr > 0.1 \Mimp$.}
\label{ch03_fig22c}
\end{center}
\end{figure}
...
\begin{figure}
\begin{center}
\includegraphics[scale=1.0]{21_SR_vinf_r3.pdf}
\caption{Relative velocity between the largest remnant and the second remnant at infinity as a function of impact angle for the simulation set \rss. Only simulations for which $\Msr > 0.1 \Mimp$ are shown.}
\label{ch03_fig21a}
\end{center}
\end{figure}
...
\caption{Relative velocity between the largest remnant and the second remnant at infinity as a function of impact angle for the simulation set \css. Only simulations for which $\Msr > 0.1 \Mimp$ are shown.}
\label{ch03_fig21b}
\end{center}
\end{figure}
...
\section{Outlook}
So far the chapter has focussed on showing direct results of the modeled collisions. Interpreting those results in the context of (terrestrial) planet formation is not straightforward, as the impactor and target bodies of a collision are also the result of previous collisions. In order to understand the role of collisions for example in planetary embryo growth, it is necessary to incorporate these results into dynamical codes which model this process \citep{Chambers:2001p2105, 2006Icar..184...39O}. As mentioned in a previous section, this has already partially been done by \cite{2010ApJ...714L..21K}, although the results from the collision simulations were only used to decide whether a collision should be treated as perfectly accreting or bouncing like rigid bodies. While this way of collision handling is superior to what other codes do, but it does for example not permit destructive collision where the target loses mass. As a next step, it would be desirable to put the results obtained here in a software library which would estimate for given impact parameters all the collisional quantities and could be directly used by N-body or statistical codes simulating the growth of planetary embryos. This process is not only important for terrestrial planet formation, but also delivers the cores required for giant planet formation \citep{1996Icar..124...62P}. Understanding the timescales of embryo growth is crucial, as they have to become massive enough to start gas accretion, before the gas disk vanishes.

An interesting aspect would also be to track compositional changes of growing bodies. Although this expands the parameter space by requiring collision simulations with target and impactor bodies with departures from standard composition, it would allow to quantify the extent of compositional changes for example for the population of next-largest bodies proposed by \cite{Asphaug:2010p3539}. He suggested that this population of bodies with masses one magnitude smaller masses than the largest bodies in the system experience collisions in an impact velocity regime which is most efficient for compositional changes. Quantifying compositional changes in general  becomes important in the light of the expected discovery of terrestrial exoplanets \citep{2010ApJ...712L..73M}.

Another recent example of coupling observations with the collisions results obtained here is the spectral observation of $SiO$-gas and warm dust out of thermal equilibrium in the HD172555 system \citep{Lisse:2009p3131}. This observation can at the moment best be explained by a recent hypervelocity impact event, which created large amount of ejecta which can now be observed. Together with dynamical considerations, the observed lower mass limits of ejecta material can put lower limits on the masses of the colliding bodies together with our results.

All this examples show that it is important to continue with the study of similar-sized collisions and deliver the results to those who need them.
