...
\end{align}
where the escape velocity is given by $\vesc = \sqrt{ 2 G \frac{\Mtar + \Mimp}{\Rtar + \Rimp} }$ and constant density bodies are assumed. Gravitational restitution happens on the gravitational timescale given by
\begin{align}
...
\begin{equation}
\sigma_{grav} \sim P_0 ~ G \rho^2 R^2
\end{equation}
...
% BUZZWORD: gravitational restitution

% TODOPLOT: compare timescales as a function of size
what do we simplify? give arguments why this is alright

scale-invariant physics: collision timescale (if vimp scaled to vesc)
collapse
basics hydro without viscosity and trivial EOS


scale variant physics:
compressibility
viscosity
EOS (small effect)
shocks
...
viscosity: Asphaug $\sigma \sim P_0 \sim G \rho^2 R^2$ -> $\nu = \sigma / \epsilon$ -> 
...
orbital time scale in solar system (change of background gravity)\\
