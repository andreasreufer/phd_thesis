...
\graphicspath{{./03figs/}}
In cratering collisions, where the impactor is much smaller than the target, the impact process can be reduced to a point source of energy, comparable to an explosion \citep{Melosh:2007p3502}. The impactor hits the target with its full volume for all impact angles, except for very grazing angles. 



The target can be simplified as a flat surface of infinite size, as it is only modified on a scale a few times bigger than the impactor. 



The result of such a collision is in almost any case a crater, expect for very grazing encounters. Scaling laws exist which for the crater depth and shape \citep{Holsapple:1993p3018}.




In similar-sized collisions, where the target has a mass in the same order of magnitude as the impactor, the collision becomes more complex. The target is altered on it entire scale and the outcome of the collision depends on the geometry of the 


The target is only altered on a scale a few times bigger than the impactor. In a similar-sized collision (\SSC), craters 


In cratering collisions, the impactor is much smaller than the target and the problem reduces to a point source event \citep{Melosh:2007p3502}. 
...
Collision between bodies of similar sizes 
...
\caption{Accretion efficiency $\xi$ for the \emph{r3} simulation set with bodies of chondritic composition (compare figure \ref{ch03_fig08a}. Each shows a different magnitude of target mass, each column a different mass ratio.}
\label{ch03_fig08b}
\end{center}
\end{figure}
...
\caption{Accretion efficiency $\xi$ for the \emph{r3} simulation set with bodies of icy composition (compare figure \ref{ch03_fig08b}).}
\label{ch03_fig08c}
\end{center}
\end{figure}
